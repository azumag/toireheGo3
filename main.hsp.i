...
loop@hsp
repeat@hsp 25
table(cnt@hsp+25)=bsi
bsi+=0.1*1000
loop@hsp
repeat@hsp 25
gyaktable.cnt@hsp=csi
csi+=0.1*1000
loop@hsp
repeat@hsp 25
gyaktable(cnt@hsp+25)=dsi
dsi-=0.1*1000
loop@hsp
...
tuyos=1
if@hsp tuyos=1:kasurikaunter=1000000000:dansu=6:shotlv=5
...
buffer@hsp 10,,
picload@hsp "haikei2men.bmp",1
buffer@hsp 11,,
picload@hsp "dona1-3.bmp",1
buffer@hsp 12,,
picload@hsp "stars2.bmp",1
buffer@hsp 13,,
picload@hsp "bg_chip.bmp",1
buffer@hsp 14,,
picload@hsp "chainguard.gif",1
...
dmloadfname "sea2.mid", 5
dmloadfname "stage2.mid",6
dmloadfname "katyhilla.mid",8
dmloadfname "katyhill_intro.mid",7
dmloadfname "Done_Gone.mid",10
...
ds_load "poi2.wav",7
ds_load "kin.wav",8
ds_load "boss01c.wav",9
ds_load "bishao.wav",10
ds_load "laser03.wav",11
ds_load "gasha.wav",12
mmload@hsp "ka.mp3",11
mmload@hsp "vman.mp3",12
mmload@hsp "vman2.mp3",14
mmload@hsp "end2.mp3",13
mmload@hsp "orange.mp3",15,1
...
tx.cnt@hsp=(int@hsp(exa)-5)*1000 :ty.cnt@hsp=int@hsp(eya+10)*1000		
txv.cnt@hsp=0 : tyv.cnt@hsp=0			
...
tx.cnt@hsp=(int@hsp(exa)-5)*1000 :ty.cnt@hsp=int@hsp(eya+110)*1000		
txv.cnt@hsp=0 : tyv.cnt@hsp=0			
...
dmstop
mmplay@hsp 13
gook=1:if@hsp stage=3:goto@hsp *stagecla3
disbullet(10)
}
}
}
...
if@hsp objtimek3.cnt@hsp>=100:ty.cnt@hsp+=5*1000:objtimek.cnt@hsp-=2
...
objtimek.cnt@hsp+=1
if@hsp (180-objtimek.cnt@hsp*5)<=0:ds_play 12:objtimek.cnt@hsp=0
...
gmode@hsp 2,143,25
grotate@hsp 14,202,0,3.14,143,25
}
...
if@hsp ts.cnt@hsp=17|ts.cnt@hsp=18|ts.cnt@hsp=22|ts.cnt@hsp=23|ts.cnt@hsp=25|ts.cnt@hsp=31|ts.cnt@hsp=32|ts.cnt@hsp=33|ts.cnt@hsp=36|ts.cnt@hsp=38|ts.cnt@hsp=39|ts.cnt@hsp=40{
targets=cnt@hsp
break@hsp
}
loop@hsp
...
if@hsp ts.cnt@hsp=6|ts.cnt@hsp=12|ts.cnt@hsp=25|ts.cnt@hsp=39|ts.cnt@hsp=40{
if@hsp mutekif>=2:break@hsp	
gosub@hsp *mycharhit	
}
if@hsp ts.cnt@hsp=2|ts.cnt@hsp=7|ts.cnt@hsp=10|ts.cnt@hsp=13|ts.cnt@hsp=19|ts.cnt@hsp=20|ts.cnt@hsp=24|ts.cnt@hsp=26|ts.cnt@hsp=27|ts.cnt@hsp=37|ts.cnt@hsp=43{
...
if@hsp ts.cnt@hsp=40|ts.cnt@hsp=44:shotatatta=1
...
pos@hsp 200,100:picload@hsp "morasita.bmp",1
...
cls@hsp:stage=4:turn=0:titlex=0:bos=0:cls@hsp:goto@hsp *gameov
}
loop@hsp
